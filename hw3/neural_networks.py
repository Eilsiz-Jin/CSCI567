...
class linear_layer:
    ...
    def forward(self, X):
        ...
        forward_output = X @ self.params['W'] + self.params['b']  # N-by-output_D
...
class relu:
    ...
    def forward(self, X):
        ...
        self.mask = (X > 0).astype(float)  # mask is 1 where X > 0, and 0 otherwise
        forward_output = X * self.mask  # relu(X) mask
        return forward_output
    ...
    def backward(self, X, grad):
        ...
        backward_output = grad * self.mask  # only pass the gradient where X > 0
